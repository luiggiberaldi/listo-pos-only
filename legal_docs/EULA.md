# CONTRATO DE LICENCIA DE USUARIO FINAL (EULA)
## LISTO POS - Sistema de Punto de Venta
**Versión 1.0 | Fecha de Vigencia: Enero 2026**

---

## IDENTIFICACIÓN DEL DESARROLLADOR

| Campo | Información |
|-------|-------------|
| **Desarrollador** | Luigi Beraldi |
| **Cédula de Identidad** | V-24.457.713 |
| **Dirección** | El Socorro, Calle Sucre, Casa #144, Valencia |
| **Ciudad/Estado** | Valencia, Estado Carabobo, Venezuela |
| **Marca Comercial** | LISTO POS (Desde 2025) |
| **Sitio Web** | www.kiplystart.com |

---

## 1. ACEPTACIÓN DEL CONTRATO

Al instalar, copiar o utilizar de cualquier forma el software LISTO POS (en adelante "el Software"), usted (en adelante "el Usuario" o "el Licenciatario") acepta quedar obligado por los términos de este Contrato de Licencia de Usuario Final.

**SI NO ESTÁ DE ACUERDO CON ESTOS TÉRMINOS, NO INSTALE NI UTILICE EL SOFTWARE.**

---

## 2. CONCESIÓN DE LICENCIA

### 2.1 Licencia Limitada
El desarrollador Luigi Beraldi le concede una licencia **NO EXCLUSIVA, INTRANSFERIBLE y LIMITADA** para:
- Instalar y usar el Software en **UN (1) SOLO EQUIPO** (computador o terminal).
- Utilizar el Software únicamente para fines comerciales legítimos de su negocio.

### 2.2 Identificador Único de Hardware (HWID)
El Software está vinculado criptográficamente al identificador único de hardware del equipo donde se instaló originalmente. **Esta licencia NO es transferible a otro equipo** sin autorización expresa del desarrollador.

---

## 3. RESTRICCIONES DE USO

El Usuario **NO PUEDE**:

1. **Copiar, duplicar o clonar** el Software para su uso en equipos adicionales.
2. **Modificar, descompilar, desensamblar** o realizar ingeniería inversa del código fuente.
3. **Sublicenciar, alquilar, prestar o transferir** la licencia a terceros.
4. **Remover o alterar** avisos de propiedad intelectual, marcas o logos del Software.
5. **Utilizar el Software** para actividades ilícitas o contrarias a la ley venezolana.

---

## 4. PROPIEDAD INTELECTUAL

### 4.1 Derechos Reservados
El Software, incluyendo pero no limitado a su código fuente, diseño, gráficos, interfaces y documentación, es propiedad exclusiva de **Luigi Beraldi** y está protegido por:
- Ley sobre el Derecho de Autor de Venezuela (Gaceta Oficial N° 4.638).
- Tratados internacionales de propiedad intelectual.

### 4.2 Marcas Comerciales
"LISTO POS" y sus logos asociados son marcas comerciales del desarrollador Luigi Beraldi. El Usuario no adquiere ningún derecho sobre estas marcas mediante el uso del Software.

---

## 5. LIMITACIÓN DE RESPONSABILIDAD

### 5.1 Sin Garantías
El Software se proporciona "TAL CUAL" (AS IS), sin garantías de ningún tipo, expresas o implícitas, incluyendo pero no limitadas a garantías de comerciabilidad o idoneidad para un propósito particular.

### 5.2 Exclusión de Daños
El desarrollador **NO SERÁ RESPONSABLE** por:
- Pérdida de datos, ingresos o ganancias.
- Daños directos, indirectos, incidentales o consecuentes.
- Fallas causadas por cortes eléctricos, virus informáticos o mal uso.

### 5.3 Responsabilidad del Usuario
El Usuario es **ÚNICO RESPONSABLE** de:
- Mantener respaldos periódicos de su información.
- Contar con sistemas de respaldo eléctrico (UPS) para proteger sus datos.
- Verificar la exactitud de los cálculos y reportes generados.

---

## 6. TERMINACIÓN

### 6.1 Por Incumplimiento
Esta licencia se termina automáticamente si el Usuario incumple cualquier término de este contrato.

### 6.2 Efectos de la Terminación
Al terminar la licencia, el Usuario debe:
- Cesar inmediatamente todo uso del Software.
- Eliminar todas las copias del Software de sus equipos.

---

## 7. DISPOSICIONES GENERALES

### 7.1 Ley Aplicable
Este contrato se rige por las leyes de la República Bolivariana de Venezuela.

### 7.2 Jurisdicción
Cualquier controversia será sometida a los tribunales competentes de la ciudad de Valencia, Estado Carabobo, Venezuela.

### 7.3 Divisibilidad
Si alguna disposición de este contrato es declarada inválida, las demás disposiciones permanecerán en pleno vigor.

---

## 8. CONTACTO

Para consultas sobre licenciamiento y soporte técnico:

| Canal | Información |
|-------|-------------|
| **Correo Electrónico** | TechSupport@kiplystart.com |
| **Teléfono / WhatsApp** | +58 412-405-1793 |
| **Sitio Web** | www.kiplystart.com |

---

**AL HACER CLIC EN "ACEPTO" O AL INSTALAR EL SOFTWARE, USTED RECONOCE QUE HA LEÍDO, ENTENDIDO Y ACEPTA ESTAR OBLIGADO POR LOS TÉRMINOS DE ESTE CONTRATO.**

---

*© 2025-2026 Luigi Beraldi. Todos los derechos reservados.*
*LISTO POS es una marca comercial de Luigi Beraldi.*
*Documento generado automáticamente. Última actualización: Enero 2026.*
