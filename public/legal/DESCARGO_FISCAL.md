# DESCARGO DE RESPONSABILIDAD FISCAL
## LISTO POS - Declaración ante Autoridades Tributarias
**Versión 1.1 | Fecha de Vigencia: Enero 2026**

---

## IDENTIFICACIÓN DEL DESARROLLADOR

| Campo | Información |
|-------|-------------|
| **Desarrollador** | Luigi Beraldi |
| **Cédula de Identidad** | V-24.457.713 |
| **Dirección** | El Socorro, Calle Sucre, Casa #144, Valencia |
| **Ciudad/Estado** | Valencia, Estado Carabobo, Venezuela |
| **Marca Comercial** | LISTO POS (Desde 2025) |

---

## ⚠️ AVISO IMPORTANTE

**LISTO POS ES UN SOFTWARE DE GESTIÓN ADMINISTRATIVA Y NO CONSTITUYE UN SISTEMA DE FACTURACIÓN FISCAL AUTORIZADO POR EL SERVICIO NACIONAL INTEGRADO DE ADMINISTRACIÓN ADUANERA Y TRIBUTARIA (SENIAT).**

---

## 1. NATURALEZA DEL SOFTWARE

### 1.1 Herramienta Administrativa
LISTO POS es una herramienta de **gestión interna** diseñada para:
- Registrar transacciones comerciales de forma organizada.
- Generar reportes de ventas para control interno del negocio.
- Administrar inventario y flujo de caja.
- Facilitar la operación diaria del punto de venta.

### 1.2 NO es Facturación Fiscal
El Software **NO ESTÁ DISEÑADO NI AUTORIZADO** para:
- Emitir facturas con valor fiscal ante el SENIAT.
- Sustituir las obligaciones de facturación legal del contribuyente.
- Generar documentos que cumplan con la normativa de la Providencia Administrativa N° SNAT/2011/0071.

---

## 2. RESPONSABILIDAD DEL USUARIO

### 2.1 Obligaciones Tributarias
El Usuario reconoce y acepta que es **ÚNICO Y EXCLUSIVO RESPONSABLE** de:

1. **Cumplir con todas las obligaciones tributarias** establecidas en el Código Orgánico Tributario y demás leyes aplicables.
2. **Emitir facturas fiscales** a través de los medios autorizados por el SENIAT (impresoras fiscales, facturación electrónica, etc.).
3. **Declarar y pagar impuestos** (IVA, ISLR, y otros) en los plazos establecidos.
4. **Mantener los libros contables** exigidos por la ley.
5. **RESPONSABILIDAD CAMBIARIA E IGTF:** El Usuario es el único responsable de la correcta actualización, configuración y aplicación de las tasas de cambio oficiales (BCV) o de mercado dentro del sistema, así como del cálculo, cobro y declaración del IGTF (Impuesto a las Grandes Transacciones Financieras) según la normativa legal vigente. El Desarrollador no asume responsabilidad alguna por pérdidas económicas, multas o discrepancias derivadas de errores en la tasa de cambio o en el cálculo de impuestos.

### 2.2 Verificación de Datos
El Usuario es responsable de:
- Verificar la exactitud de todos los montos y cálculos generados por el Software.
- Conciliar los reportes del Software con su sistema de facturación fiscal oficial.
- Detectar y corregir cualquier discrepancia antes de presentar declaraciones.

---

## 3. EXENCIÓN DE RESPONSABILIDAD

### 3.1 Multas y Sanciones
El desarrollador Luigi Beraldi **NO ASUME NINGUNA RESPONSABILIDAD** por:
- Multas impuestas por el SENIAT o cualquier autoridad tributaria.
- Sanciones derivadas del incumplimiento de obligaciones fiscales.
- Cierres temporales o definitivos del establecimiento por razones tributarias.
- Intereses moratorios o recargos por declaraciones tardías o incorrectas.

### 3.2 Errores de Cálculo
El desarrollador **NO SERÁ RESPONSABLE** por:
- Errores en el cálculo de impuestos (IVA, retenciones, etc.).
- Discrepancias entre los reportes del Software y la realidad contable.
- Pérdidas económicas derivadas de decisiones basadas en datos del Software.

### 3.3 Pérdida de Datos
El desarrollador **NO SERÁ RESPONSABLE** por pérdida de información causada por:
- Cortes de energía eléctrica (el Usuario debe contar con UPS).
- Fallas de hardware del equipo del Usuario.
- Virus informáticos o ataques cibernéticos.
- Errores de operación por parte del Usuario.

---

## 4. RECOMENDACIONES IMPORTANTES

### 4.1 Sistema de Respaldo Eléctrico (UPS)
**ES OBLIGATORIO** que el Usuario cuente con un Sistema de Alimentación Ininterrumpida (UPS) para:
- Proteger los datos ante cortes de luz.
- Permitir el cierre seguro del sistema en emergencias.
- Evitar corrupción de la base de datos local.

### 4.2 Respaldos de Información
Se recomienda encarecidamente:
- Realizar respaldos diarios de la información.
- Almacenar copias de seguridad en ubicaciones externas.
- Verificar periódicamente la integridad de los respaldos.

### 4.3 Asesoría Profesional
El Usuario debe consultar con un **contador público colegiado** para:
- Determinar sus obligaciones tributarias específicas.
- Configurar correctamente las tasas impositivas en el Software.
- Interpretar los reportes generados para fines fiscales.

---

## 5. DECLARACIÓN DEL USUARIO

Al utilizar LISTO POS, el Usuario declara que:

1. Ha leído y comprendido este descargo de responsabilidad.
2. Entiende que el Software es una herramienta administrativa, no fiscal.
3. Asume la total responsabilidad de sus obligaciones tributarias.
4. Exime al desarrollador Luigi Beraldi de cualquier consecuencia derivada del uso del Software en materia fiscal.

---

## 6. BASE LEGAL

Este descargo se fundamenta en:
- Código Orgánico Tributario de Venezuela.
- Ley de Impuesto al Valor Agregado (IVA).
- Providencias Administrativas del SENIAT sobre facturación.
- Código Civil de Venezuela (artículos sobre responsabilidad contractual).

---

## 7. CONTACTO

Para consultas:

| Canal | Información |
|-------|-------------|
| **Correo Electrónico** | TechSupport@kiplystart.com |
| **Teléfono / WhatsApp** | +58 412-405-1793 |
| **Sitio Web** | www.kiplystart.com |

---

**EL USO CONTINUADO DEL SOFTWARE IMPLICA LA ACEPTACIÓN TOTAL DE ESTE DESCARGO DE RESPONSABILIDAD FISCAL.**

---

*© 2025-2026 Luigi Beraldi. Todos los derechos reservados.*
*LISTO POS es una marca comercial de Luigi Beraldi.*
*Documento generado automáticamente. Última actualización: Enero 2026.*
