...
import { X, Scale, DollarSign, Banknote, Check, AlertTriangle, Lock, Droplet } from 'lucide-react';
      ...
      if (pesoFinal > stockActual) {
          if (!permitirSinStock) {
              ...
              Swal.fire({
                  ...
                  icon: 'error',
                  title: producto.tipoUnidad === 'litro' ? 'Límite Excedido' : 'Peso Excedido',
                  text: `Solo tienes ${stockActual.toFixed(3)} ${producto.tipoUnidad === 'litro' ? 'Lt' : 'Kg'} disponibles.`
              });
              return;
          } else {
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col">
        ...
        <div className="bg-slate-800 p-4 flex justify-between items-center text-white shrink-0 relative">
            <h2 className="font-bold text-lg flex items-center gap-2">
                {producto.tipoUnidad === 'litro' ? <Droplet className="text-blue-400"/> : <Scale className="text-blue-400"/>}
                {producto.tipoUnidad === 'litro' ? 'Medición de Volumen' : 'Balanza Digital'}
            </h2>
            ...
            <div className={`absolute right-12 top-4 text-xs font-mono px-2 py-0.5 rounded border ${isStockEmpty ? 'bg-red-900 text-red-200 border-red-700' : (isStockLow ? 'bg-amber-900 text-amber-200 border-amber-700' : 'bg-slate-700 text-slate-300 border-slate-600')}`}>
                Stock: {stockActual.toFixed(3)} {producto.tipoUnidad === 'litro' ? 'Lt' : 'Kg'}
            </div>
        ...
        <div className="p-6 space-y-5" onKeyDown={handleContainerKeyDown}>
          <div className="text-center mb-4">
            <h3 className="text-xl font-black text-slate-800 dark:text-white leading-tight">{producto.nombre}</h3>
            <div className="text-sm text-slate-500 font-bold mt-1">Precio: ${precioPorKg.toFixed(2)} / {producto.tipoUnidad === 'litro' ? 'Lt' : 'Kg'}</div>
          </div>
          ...
          <div className="relative">
              <label className="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase mb-1 block">
                {producto.tipoUnidad === 'litro' ? 'Volumen Resultante (Lt)' : 'Peso Resultante (Kg)'}
              </label>
              <div className="relative">
                  {producto.tipoUnidad === 'litro' ? <Droplet size={20} className="absolute left-3 top-3.5 text-blue-500" /> : <Scale size={20} className="absolute left-3 top-3.5 text-blue-500" />}
                  ...
                  <span className="absolute right-4 top-4 text-xs font-bold text-blue-400">{producto.tipoUnidad === 'litro' ? 'Lt' : 'Kg'}</span>
              </div>
          </div>
