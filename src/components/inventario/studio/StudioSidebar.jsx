...
export const StudioSidebar = ({ config, setConfig, mockData, setMockData, handlePaperChange }) => {
    return (
        <div className="w-80 border-r border-border-subtle dark:border-slate-800 p-6 overflow-y-auto bg-app-light/50 dark:bg-slate-900/30 font-sans space-y-8 flex-shrink-0">

            {/* DATOS DE PRUEBA */}
            <div className="bg-surface-light dark:bg-slate-800 p-4 rounded-xl border border-border-subtle dark:border-slate-700 shadow-sm space-y-3">
                <label className="text-xs font-bold text-content-secondary uppercase tracking-wider flex items-center gap-2 mb-2">
                    <Monitor size={14} /> Datos de Prueba
                </label>

                <div>
                    <label className="text-[10px] font-bold text-content-secondary block mb-1">Nombre Producto</label>
                    <input
                        type="text"
                        value={mockData.nombre}
                        onChange={(e) => setMockData({ ...mockData, nombre: e.target.value })}
                        className="w-full text-xs p-2 rounded border border-border-subtle dark:border-slate-700 bg-app-light dark:bg-slate-900"
                    />
                </div>

                <div className="flex gap-2">
                    <div className="flex-1">
                        <label className="text-[10px] font-bold text-content-secondary block mb-1">Precio $</label>
                        <input
                    ...
                    <div className="flex-1">
                        <label className="text-[10px] font-bold text-content-secondary block mb-1">SKU</label>
                        <input
                            type="text"
            ...
            <div className="space-y-3">
                <label className="text-xs font-bold text-content-secondary uppercase tracking-wider flex items-center gap-2">
                    <LayoutTemplate size={14} /> Plantilla Visual
                </label>
                <div className="flex gap-2 p-1 bg-app-light dark:bg-slate-800 rounded-xl">
                    {['modern', 'boutique'].map(t => (
                        <button
                            key={t}
                            onClick={() => setConfig({ ...config, designTemplate: t })}
                            className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all capitalize ${config.designTemplate === t ? 'bg-surface-light dark:bg-slate-700 text-indigo-600 shadow-sm' : 'text-content-secondary hover:text-content-main'}`}
                        >
                            {t}
                        </button>
                    ))}
                ...
                <div className="flex gap-2 p-1 bg-app-light dark:bg-slate-800 rounded-xl">
                    {['usd', 'bs', 'mix'].map(m => (
                        <button
                            ...
                            className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${config.moneda === m ? 'bg-surface-light dark:bg-slate-700 text-indigo-600 shadow-sm' : 'text-content-secondary hover:text-content-main'}`}
                        >
                            {m.toUpperCase()}
                        </button>
                    ))}
                ...
                <div className="flex gap-2 p-1 bg-app-light dark:bg-slate-800 rounded-xl">
                    {['helvetica', 'times', 'courier'].map(f => (
                        <button
                            ...
                            className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all capitalize ${config.fontFamily === f ? 'bg-surface-light dark:bg-slate-700 text-indigo-600 shadow-sm' : 'text-content-secondary hover:text-content-main'}`}
                            style={{ fontFamily: f }}
                        >
                            {f}
                        </button>
            ...
            <div className="space-y-4">
                ...
                {/* Title Size */}
                <div className="bg-surface-light dark:bg-slate-800 p-4 rounded-xl border border-border-subtle dark:border-slate-700">
                    <div className="flex justify-between mb-2">
                        <span className="text-xs font-bold text-content-secondary">Título Producto</span>
                ...
                <div className="bg-surface-light dark:bg-slate-800 p-4 rounded-xl border border-border-subtle dark:border-slate-700">
                    <div className="flex justify-between mb-2">
                        <span className="text-xs font-bold text-content-secondary">Precio Principal</span>
                ...
                {config.moneda === 'mix' && (
                    <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 animate-in fade-in slide-in-from-top-2">
                        <div className="flex justify-between mb-2">
                            <span className="text-xs font-bold text-content-secondary">Precio Secundario</span>
                ...
                <div className="bg-surface-light dark:bg-slate-800 p-4 rounded-xl border border-border-subtle dark:border-slate-700">
                    <div className="flex justify-between mb-2">
                        <span className="text-xs font-bold text-content-secondary">Metadata (SKU/Fecha)</span>
                ...
                <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800 space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        <button
                            ...
                            className={`text-[10px] px-2 py-1 rounded border transition-all font-bold ${config._simulateHierarchy ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-surface-light text-content-secondary border-border-subtle'}`}
                        >
                    ...
                    <div>
                        <div className="flex justify-between mb-1">
                            <span className="text-[10px] font-bold text-content-secondary">Tamaño Badge</span>
                    ...
                    <div>
                        <div className="flex justify-between mb-1">
                            <span className="text-[10px] font-bold text-content-secondary">Posición Y</span>
                ...
                {/* AJUSTE FINO */}
                <div className="space-y-4 border-t border-border-subtle dark:border-slate-800 pt-4">
                    ...
                    <div>
                        <div className="flex justify-between mb-1">
                            <span className="text-[10px] font-bold text-content-secondary">Título Y</span>
                    ...
                    <div>
                        <div className="flex justify-between mb-1">
                            <span className="text-[10px] font-bold text-content-secondary">Precio $ Y</span>
                    ...
                    {config.moneda === 'mix' && (
                        <div>
                            <div className="flex justify-between mb-1">
                                <span className="text-[10px] font-bold text-content-secondary">Precio Bs Y</span>
                ...
                <div className="space-y-2">
                    {config.designTemplate === 'modern' && (
                        <button onClick={() => setConfig({ ...config, showBorder: !config.showBorder })} className="w-full flex items-center justify-between p-3 rounded-xl bg-surface-light dark:bg-slate-800 border border-border-subtle dark:border-slate-700 hover:border-indigo-300 transition-all">
                            <span className="text-sm text-content-main dark:text-slate-300 flex items-center gap-2"><Square size={14} /> Mostrar Borde</span>
                    ...
                    <button onClick={() => setConfig({ ...config, showDate: !config.showDate })} className="w-full flex items-center justify-between p-3 rounded-xl bg-surface-light dark:bg-slate-800 border border-border-subtle dark:border-slate-700 hover:border-indigo-300 transition-all">
                        <span className="text-sm text-content-main dark:text-slate-300 flex items-center gap-2"><Calendar size={14} /> Fecha Impresión</span>
                    ...
                    <button onClick={() => setConfig({ ...config, showCode: !config.showCode })} className="w-full flex items-center justify-between p-3 rounded-xl bg-surface-light dark:bg-slate-800 border border-border-subtle dark:border-slate-700 hover:border-indigo-300 transition-all">
                        <span className="text-sm text-content-main dark:text-slate-300 flex items-center gap-2"><BarChart2 size={14} /> Código SKU</span>
