...
export default function ProductPricing({ form, updateField, tasa, getFactores, showCosts = false }) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex p-1 bg-app-light dark:bg-slate-900 rounded-xl border border-border-subtle dark:border-slate-800">
                <button
                    type="button"
                    onClick={() => updateField('tipoUnidad', 'unidad')}
                    className={`flex-1 py-2.5 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-all ${form.tipoUnidad === 'unidad' ? 'bg-surface-light dark:bg-slate-800 text-content-main dark:text-white shadow-sm ring-1 ring-border-subtle dark:ring-slate-700' : 'text-content-secondary hover:text-content-main'}`}
                >
                ...
                <button
                    ...
                    className={`flex-1 py-2.5 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-all ${form.tipoUnidad === 'peso' ? 'bg-surface-light dark:bg-slate-800 text-content-main dark:text-white shadow-sm ring-1 ring-border-subtle dark:ring-slate-700' : 'text-content-secondary hover:text-content-main'}`}
                >
            ...
            {showCosts && (
                <div className="bg-app-light dark:bg-slate-900/50 p-5 rounded-2xl border border-border-subtle dark:border-slate-800">
                    <div className="flex items-center justify-between mb-4">
                        <label className="text-xs font-bold text-content-secondary uppercase tracking-wider">
                            Costo de Compra <span className="text-content-secondary/70 font-normal">({getNombreJerarquia()})</span>
                        </label>
                        <TrendingUp size={16} className="text-content-secondary/50" />
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div className="relative group">
                            <div className="absolute left-3 top-3.5 text-content-secondary font-bold">$</div>
                            <input
                                type="number"
                                step="any"
                                onWheel={preventScroll}
                                className="w-full pl-8 p-3 bg-surface-light dark:bg-slate-950 border border-border-subtle dark:border-slate-800 rounded-xl font-bold text-lg text-content-main dark:text-white outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all placeholder:text-content-secondary/50"
                                placeholder="0.00"
                                value={costoInputUSD}
                                onChange={e => handleCostoUSD(e.target.value)}
                            />
                            <div className="absolute right-3 top-4 text-[10px] font-bold text-content-secondary/50">USD</div>
                        </div>
                        <div className="relative group">
                            <div className="absolute left-3 top-3.5 text-content-secondary font-bold">Bs</div>
                            <input
                                type="number"
                                step="any"
                                onWheel={preventScroll}
                                className="w-full pl-9 p-3 bg-surface-light dark:bg-slate-950 border border-border-subtle dark:border-slate-800 rounded-xl font-bold text-lg text-content-main dark:text-slate-300 outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all placeholder:text-content-secondary/50"
                                placeholder="0.00"
                                value={costoInputBS}
            ...
            {/* CHECKBOX IVA */}
            <label className={`flex items-center gap-4 p-4 border rounded-2xl cursor-pointer transition-all ${form.aplicaIva ? 'border-primary/30 bg-primary/5 dark:bg-blue-900/10 dark:border-blue-800' : 'border-border-subtle bg-surface-light hover:bg-app-light dark:bg-slate-950 dark:border-slate-800'}`}>
                ...
                <div className="flex-1">
                    <p className="font-bold text-sm text-content-main dark:text-slate-200">Aplicar Impuesto (IVA)</p>
                    <p className="text-xs text-content-secondary mt-0.5">{form.aplicaIva ? 'Se sumará 16% al total en caja' : 'Producto exento de impuestos'}</p>
                </div>
            </label>
        </div>
    );
