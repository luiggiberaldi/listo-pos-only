...
export default function EmployeeRegistrationForm({ formState, setFormState, onSubmit, readOnly, maxEmpleados, currentCount, isBasicPlan }) {
    ...
    const atLimit = maxEmpleados && maxEmpleados !== Infinity && currentCount >= maxEmpleados;
    return (
        <div className="bg-surface-light dark:bg-surface-dark p-8 rounded-[2rem] shadow-xl shadow-slate-200/50 dark:shadow-black/20 border border-border-subtle dark:border-slate-800 h-fit sticky top-8 relative overflow-hidden group transition-all duration-300">
            ...
            <div className="flex items-center gap-5 mb-8 pb-6 border-b border-border-subtle dark:border-slate-800/50 relative z-10">
                ...
                <div>
                    ...
                    <p className="text-sm text-content-secondary font-medium mt-0.5">
                        {atLimit ? `Límite alcanzado (${currentCount}/${maxEmpleados})` : `Añadir personal al equipo${maxEmpleados && maxEmpleados !== Infinity ? ` (${currentCount}/${maxEmpleados})` : ''}`}
                    </p>
                </div>
            </div>
            ...
            {atLimit ? (
                <div className="text-center py-8 space-y-3">
                    <p className="text-slate-400 text-sm font-bold">Tu plan permite máximo {maxEmpleados} empleados.</p>
                    <p className="text-slate-500 text-xs">Actualiza a Minimarket para equipos más grandes.</p>
                </div>
            ) : (
                <form onSubmit={onSubmit} className="space-y-6 relative z-10">
                    ...
                    {!isBasicPlan && formState.rol !== 'Cajero' && formState.rol !== 'Encargado' && (
                    ...
                    {/* CONFIGURACIÓN DE NÓMINA — Solo Minimarket (plan con ROLES completo) */}
                    {!isBasicPlan && (
